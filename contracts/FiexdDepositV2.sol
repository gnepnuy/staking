...
contract FiexdDeposit is Durations{
  ...
  function extension(uint256 duration) external {
    ...
    rewardPool.amount += depositSlip.reward;
    rewardPool.duration += depositSlip.duration;
    if(rewardPool.startTime == 0){
      rewardPool.startTime = deadline;
    }else{
      rewardPool.startTime += block.timestamp - deadline;
    }

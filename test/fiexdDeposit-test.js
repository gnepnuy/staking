...
const fiexdDeposit_address = "0x12ef6d1464dc3dbB2c1e61f6BA0Ff5Fafcb5B75D";//v3
...
async function test(){
  ...
  //查询延长质押限制次数
  const extensionCountLimit = await fiexdDeposit.extensionCountLimit();
  console.log("extensionCountLimit:",extensionCountLimit.toString());

  //查询用户已延长质押次数
  const userExtensionCount = await fiexdDeposit.viewUserExtensionCount(wallet.address);
  console.log("userExtensionCount:",userExtensionCount.toString());
